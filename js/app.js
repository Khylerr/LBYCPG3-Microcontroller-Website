...
/* 
========================================= 
    PART 4: NEWSLETTER FORM VALIDATION
========================================= 
*/
function initNewsletterForm() {
    ...
    if (form && formContainer && successMessage) {
        ...
        form.addEventListener('submit', function(event) {
            ...
            // Flag to track if the entire form is ready to submit
            let isValid = true;

            // Ensure the user didn't just submit empty spaces
            if (nameInput.value.trim() === '') {
                nameInput.classList.add('is-invalid'); // Bootstrap error styling
                isValid = false;
            } else {
                nameInput.classList.remove('is-invalid');
                nameInput.classList.add('is-valid'); // Bootstrap success styling
            }

            // Regex pattern to check for a standard "text@text.domain" format
            const emailPattern = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;

            // Fail validation if the field is empty OR doesn't match the email pattern
            ...
            // 3. Dynamic DOM Update on Success
            if (isValid) {
                // Safely inject the user's text into the success message spans
                document.getElementById('display-name').textContent = nameInput.value.trim();
                document.getElementById('display-email').textContent = emailInput.value.trim();
                
                // Swap the visibility states: hide the form, show the success message
                formContainer.style.display = 'none';
                successMessage.style.display = 'block';
            }
        });
        
        // Remove validation errors in real-time as the user types
